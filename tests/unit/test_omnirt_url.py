...
def test_https_becomes_wss():
    assert (
        ...
        == "wss://omnirt.example.com/v1/avatar/musetalk"
    )
...
def test_ws_endpoint_passthrough():
    assert (
        ...
        == "ws://omnirt:9000/v1/avatar/wav2lip"
    )
...
def test_endpoint_with_basepath_preserved():
    assert (
        ...
        == "ws://gw.example.com/omnirt/v1/avatar/flashtalk"
    )
...
def test_resolve_default_when_nothing_set():
    ...
    assert resolve_synthesis_ws_url("musetalk", s).endswith("/v1/avatar/musetalk")
