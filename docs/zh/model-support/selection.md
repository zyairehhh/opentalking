...
- 想用摄像头或自拍视频驱动数字人表情和头动：从 FasterLivePortrait 的视频克隆开始。
- 想同时验证本地语音识别、本地语音合成和 QuickTalk：看[本地语音 + QuickTalk](../recipes/local-quicktalk-audio.md)。
...
- QuickTalk local。
- MuseTalk local。
- MuseTalk / Wav2Lip / QuickTalk / FasterLivePortrait 的 OmniRT 单模型服务。
...
| 摄像头视频驱动 | FasterLivePortrait | `omnirt` | 用摄像头或上传 driving video 驱动 source 数字人 |
...
如果当前目标是文档读者快速上手，推荐路线是：Mock -> QuickTalk 或 Wav2Lip -> OmniRT。需要自拍视频或摄像头驱动时，在 OmniRT 路径中加入 FasterLivePortrait。
