...
使开发者可以从 Mock 验证开始，逐步切换到 Wav2Lip、QuickTalk、FasterLivePortrait、MuseTalk、FlashTalk 或
OmniRT 等真实模型和推理后端。
...
- [快速开始](quick-start/index.md) —— 首次运行和 Mock 验证。
- [模型支持](model-support/index.md) —— 选择模型、后端和部署路径。
- [模型部署](deployment/index.md) —— 模型部署与 TTS 权重准备。
- [Avatar 模型](avatar_models/index.md) —— Wav2Lip、QuickTalk、MuseTalk、FlashTalk 等。
- [语音生成模型](speech_models/index.md) —— LLM、STT、TTS 部署。
- [部署配方](recipes/index.md) —— 本地语音 + QuickTalk 等组合方案。
...
- [Usage](usage/index.md)：命令行启动、WebUI 使用、视频克隆、Avatar 配置和音色/TTS 配置。
- [Examples](examples/index.md)：AI 客服、商品讲解、课程口播等场景。
- [模型支持](model-support/index.md)：模型与后端选择，以及生产拓扑。
- [参考资料](reference/index.md)：Benchmark、性能指标和更新日志。
- [FAQ](faq.md)：安装、配置、WebRTC、模型 backend 和运行问题。
