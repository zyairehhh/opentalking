...
OpenTalking 将 Avatar 作为通用形象资产处理：一个 Avatar 可以被不同 talking-head 模型复用。
不同模型需要的缓存、模板视频、参考帧或预处理产物，由上传流程、准备脚本或首次会话自动生成。

一般可以这样理解：

- 图片 Avatar：适合快速验证，通常依赖参考图、口型检测和模型自身的驱动能力。
- 视频 Avatar：适合保留更自然的姿态和背景，但准备成本更高。
- 模型部署页会说明如何生成该模型需要的派生资产，但 Avatar 本身不应写成某个模型专属。
