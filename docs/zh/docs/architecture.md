...
flowchart TB
    ...
    subgraph "OpenTalking 服务"
      ...
      Worker[opentalking.runtime<br/>Worker 服务<br/>流水线驱动]
      Bus[(Redis<br/>或内存)]
    end
    subgraph 外部
...
| `opentalking-worker` | 驱动一个或多个会话执行渲染管线。 | `opentalking/runtime/main.py`、`opentalking/runtime/task_consumer.py`、`opentalking/runtime/server.py` |
| `apps/unified` | 单进程组合 API、Worker 与内存总线，面向开发场景。 | `apps/unified/main.py` |
...
| `speak` / `speak_flashtalk_audio` 任务 | API | Worker |
| `cancel` | API | Worker |
...
│   ├── queue_status.py    # 队列状态查询
│   └── types/             # AudioChunk、VideoFrameData 等共享类型
├── providers/
│   ├── llm/               # OpenAI 兼容流式客户端
...
│   ├── speak/             # LLM -> TTS -> 合成流水线
│   └── recording/         # 录制与离线导出
├── runtime/               # opentalking-worker 入口、Worker HTTP 服务、任务消费与时序
...
- [渲染管线](render-pipeline.md) —— 音频到视频链路的逐阶段说明。
